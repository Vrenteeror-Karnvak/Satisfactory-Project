...
int main() {
    ...
    ifstream fin("C:\\Users\\adamb\\OneDrive\\Documents\\GitHub\\Satisfactory-Project\\en-US.json");
    ofstream name_out("C:\\Users\\adamb\\OneDrive\\Documents\\GitHub\\Satisfactory-Project\\names.json");
    ofstream recipe_out("C:\\Users\\adamb\\OneDrive\\Documents\\GitHub\\Satisfactory-Project\\recipes.json");
    ...
    for (const auto& block : root) {
        ...
        if (block.value("NativeClass", "") == "/Script/CoreUObject.Class'/Script/FactoryGame.FGItemDescriptor'") {
            ...
            for (const auto& data : block["Classes"]) {
                ...
                name_data["DisplayName"] = data.value("mDisplayName", ""); // grabs the display name which is human readable

                // adds the two values to a vector for later use
                Class_Name.push_back(data.value("ClassName", ""));
                Display_Name.push_back(data.value("mDisplayName", ""));

                dataOut.push_back(name_data); // adds the collected data to the vector
            }

            // outputs the data to its file and then clears the vector
            name_out << dataOut.dump(4);
            dataOut.clear();
        }
        ...
        if (block.value("NativeClass", "") == "/Script/CoreUObject.Class'/Script/FactoryGame.FGRecipe'") {
            ...
            for (const auto& data : block["Classes"]) {
                ...
                && data.value("mProducedIn", "") != "") {
                    ...
                    dataOut.push_back(recipe_data); // adds the collected data to the vector
                }
            }
